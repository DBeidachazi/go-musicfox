...
- **Automix（歌曲间智能过渡，仅 beep 引擎）**：移植自 folia-major 的 Folia Automix（AGPL-3.0）。`[player.beep] automix = off | crossfade | automix`，建立在 gapless 预加载之上（`internal/player/beep_gapless.go`）。分三级：
  ...
  - **L2**（`automixModels = "full"`）：htdemucs 四轨分离。`internal/automix/models/stems.go` 优先用 Python 旁路（内嵌 folia 的 `htdemucs_runner.py`，为关闭 `enable_mem_reuse` 将峰值压到约 0.5GB），无 Python 时进程内 ORT（`automixStemBackend`）。每首歌分离曲尾/曲首 30 秒（`internal/player/beep_automix_models.go`，与播放同一解码器、跳转带 4 帧预滚以保证逐采样对齐），`other` 由相减得出使四轨之和精确等于原曲；`stemgesture.go`（上游 stemGesture.ts 的移植，有 golden 对拍）决定鼓/贝斯/人声的交接，`stemmix.go` 在 `Mixer` 重叠段逐采样执行（取代三频段与回声）。规划器用分离出的人声结束时刻（`TransitionTrack.VocalEnd`）替代歌词估计，并以两端窗口能覆盖的长度封顶（`Separated` / gestureRoom）。
  - 模型不进安装包：`internal/automix/models` 按清单（与 folia 相同的文件与 sha256，镜像 hf-mirror → huggingface → github，可加 `automixModelMirrors`）后台下载到 `<data>/automix/models`，也接受手动放入的文件（校验后写 `.verified` 标记）。onnxruntime 动态库取微软官方 CPU 包（1.29.0，Intel Mac 1.23.2），其 sha256 尚未钉住（只校验来源并在日志打印哈希）。端到端真实模型测试：`MUSICFOX_AUTOMIX_MODELS=<dir> go test ./internal/automix/models -run Real -v`。
