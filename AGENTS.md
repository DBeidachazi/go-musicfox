...
- **Automix（歌曲间智能过渡，仅 beep 引擎）**：移植自 folia-major 的 Folia Automix（AGPL-3.0）。`[player.beep] automix = off | crossfade | automix`，建立在 gapless 预加载之上（`internal/player/beep_gapless.go`）。分三级：
  - **L0**（纯 Go，默认）：`internal/automix` 的 `profile.go`/`signal.go`（BPM、响度、调性、段落、结尾形态）→ `chooser.go`/`planner.go`（beatCut / bassSwap / tailRide / plainBlend 与长度、落点）→ `mixer.go`（采样域等功率曲线、三频段、回声、软限幅）。播放器侧 `internal/player/beep_automix.go`：预加载时分析下一首、下载完成后分析当前曲，档案缓存于 `cache/automix`；到点在音频回调里交给 `Mixer`。
  - **L1**（`automixModels = "beat"`）：Beat This! 拍点模型。`internal/automix/beatthis.go` 是纯算术部分（梅尔频谱、1500 帧分块、峰值挑选，与上游逐位对拍，见 `testdata/beatthis_golden.json`）；模型经 `internal/automix/ort`（purego 调 onnxruntime C API，无 cgo，按 OrtApi 函数表下标调用，API 版本 17）在进程内运行，结果作为 `AnalyseOptions.Grid` 替换自相关估计。无网格的旧档案在模型就绪后重测。
  - **L2**（`automixModels = "full"`）：htdemucs 四轨分离。`internal/automix/models/stems.go` 优先用 Python 旁路（内嵌 folia 的 `htdemucs_runner.py`，为关闭 `enable_mem_reuse` 将峰值压到约 0.5GB），无 Python 时进程内 ORT（`automixStemBackend`）。每首歌分离曲尾/曲首 30 秒（`internal/player/beep_automix_models.go`；进场曲在预加载时连同曲尾一起分离，因为经过渡换上来的歌不走下载路径；与播放同一解码器、跳转带 4 帧预滚以保证逐采样对齐），`other` 由相减得出使四轨之和精确等于原曲；`stemgesture.go`（上游 stemGesture.ts 的移植，有 golden 对拍）决定鼓/贝斯/人声的交接，`stemmix.go` 在 `Mixer` 重叠段逐采样执行（取代三频段与回声）。规划器用分离出的人声结束时刻（`TransitionTrack.VocalEnd`）替代歌词估计，并以两端窗口能覆盖的长度封顶（`Separated` / gestureRoom）。
  - 模型不进安装包：`internal/automix/models` 按清单（与 folia 相同的文件与 sha256，镜像 hf-mirror → huggingface → github，可加 `automixModelMirrors`）后台下载到 `<data>/automix/models`，也接受手动放入的文件（校验后写 `.verified` 标记）。onnxruntime 动态库取微软官方 CPU 包（1.29.0，Intel Mac 1.23.2）；Windows x64 的官方 1.29.0 包已固定 sha256，其余平台包下载时记录哈希、尚未固定。端到端真实模型测试：`MUSICFOX_AUTOMIX_MODELS=<dir> go test ./internal/automix/models -run Real -v`。
